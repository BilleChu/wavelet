# OpenFinance

<div align="center">

![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)
![Python](https://img.shields.io/badge/python-3.11+-green.svg)
![TypeScript](https://img.shields.io/badge/typescript-5.0+-blue.svg)
![License](https://img.shields.io/badge/license-MIT-orange.svg)

**基于大语言模型的智能金融分析平台**

[功能特性](#功能特性) • [快速开始](#快速开始) • [技术架构](#技术架构) • [API文档](#api文档) • [部署指南](#部署指南)

</div>

---

## 📖 项目简介

OpenFinance（WaveletTrader）是一个基于大语言模型（LLM）的智能金融分析平台，旨在为投资者提供专业、智能、个性化的金融分析服务。平台融合了人工智能、知识图谱、实时数据等技术，实现从数据采集、分析、到智能问答的全链路金融服务。

### 核心价值

| 价值主张 | 描述 |
|----------|------|
| **智能分析** | 基于LLM的智能问答和深度分析能力 |
| **专业视角** | 模拟投资大师（巴菲特、达里奥等）的投资观点 |
| **实时数据** | 对接多数据源，提供实时市场数据 |
| **知识图谱** | 构建金融领域知识图谱，支持关联分析 |
| **可视化** | 丰富的图表展示，直观呈现分析结果 |

---

## ✨ 功能特性

### 🤖 智能问答

基于自然语言的智能金融问答系统，支持多种查询类型：

- **股票搜索**: "浦发银行的市盈率是多少"
- **股票分析**: "分析一下贵州茅台的投资价值"
- **行业分析**: "银行业的发展趋势如何"
- **宏观分析**: "中国GDP增长率是多少"
- **角色观点**: "巴菲特怎么看比亚迪"
- **股票排名**: "市盈率最低的银行股有哪些"

### 🎭 角色扮演

模拟投资大师的投资风格和观点：

| 角色 | 投资风格 | 核心理念 |
|------|----------|----------|
| 沃伦·巴菲特 | 价值投资 | 长期持有、安全边际、护城河 |
| 瑞·达里奥 | 全天候策略 | 风险平价、多元化配置、经济周期 |
| 凯瑟琳·伍德 | 创新投资 | 颠覆性创新、主题投资、长期成长 |

### 📊 数据分析

- **股票分析**: 估值分析、基本面分析、技术面分析
- **行业分析**: 行业对比、产业链分析、景气度跟踪
- **宏观分析**: GDP、CPI、PMI等宏观指标
- **新闻资讯**: 财经新闻、公告速递、研报精选

### 🔧 系统管理

- **Agent管理**: 智能代理的创建、配置、监控
- **技能管理**: 技能注册、生命周期管理
- **工具管理**: 工具注册、调用、编排
- **流程编排**: 工作流设计、执行、监控

---

## 🏗️ 技术架构

```
┌─────────────────────────────────────────────────────────────────────────┐
│                           OpenFinance 技术架构                            │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │                         前端应用层                                  │ │
│  │                    Next.js 14 + React 18                          │ │
│  │              TailwindCSS + Radix UI + ECharts                     │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                   │                                     │
│                                   ▼                                     │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │                         API网关层                                   │ │
│  │                      FastAPI + WebSocket                           │ │
│  │              认证授权 / 限流 / 日志 / 监控                          │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                   │                                     │
│                                   ▼                                     │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │                         业务服务层                                  │ │
│  ├─────────────┬─────────────┬─────────────┬─────────────┬──────────┤ │
│  │  Agent引擎  │  技能管理   │  工具调用   │  意图识别   │ 知识图谱  │ │
│  └─────────────┴─────────────┴─────────────┴─────────────┴──────────┘ │
│                                   │                                     │
│                                   ▼                                     │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │                         LLM引擎层                                   │ │
│  │              OpenAI / Claude / 本地模型                             │ │
│  │           LangGraph / LangChain / Prompt管理                       │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                   │                                     │
│                                   ▼                                     │
│  ┌───────────────────────────────────────────────────────────────────┐ │
│  │                         数据服务层                                  │ │
│  ├─────────────┬─────────────┬─────────────┬─────────────┬──────────┤ │
│  │  行情数据   │  财务数据   │  新闻数据   │  知识图谱   │ 向量存储  │ │
│  │   AKShare   │   Tushare   │   RSS/API   │   Neo4j     │ Milvus   │ │
│  └─────────────┴─────────────┴─────────────┴─────────────┴──────────┘ │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### 技术栈

#### 后端

| 技术 | 版本 | 用途 |
|------|------|------|
| Python | 3.11+ | 主要开发语言 |
| FastAPI | 0.109+ | Web框架 |
| LangGraph | 0.2+ | Agent编排 |
| LangChain | 0.2+ | LLM应用框架 |
| Pydantic | 2.5+ | 数据验证 |
| Redis | 7.0+ | 缓存/消息队列 |
| PostgreSQL | 15+ | 关系型数据库 |
| Neo4j | 5.0+ | 知识图谱 |
| Milvus | 2.3+ | 向量数据库 |

#### 前端

| 技术 | 版本 | 用途 |
|------|------|------|
| Next.js | 14.2+ | React框架 |
| React | 18.2+ | UI库 |
| TypeScript | 5.0+ | 类型系统 |
| TailwindCSS | 3.4+ | 样式框架 |
| Radix UI | 1.0+ | 无障碍组件库 |
| ECharts | 5.4+ | 图表库 |
| Zustand | 4.4+ | 状态管理 |
| Axios | 1.6+ | HTTP客户端 |

---

## 🚀 快速开始

### 环境要求

- Python 3.11+
- Node.js 18+
- Docker & Docker Compose (可选)
- Redis 7.0+
- PostgreSQL 15+

### 本地开发

#### 1. 克隆项目

```bash
git clone https://github.com/your-org/openfinance.git
cd openfinance
```

#### 2. 后端设置

```bash
# 创建虚拟环境
cd backend
python -m venv venv
source venv/bin/activate  # Linux/macOS
# venv\Scripts\activate  # Windows

# 安装依赖
pip install -r requirements.txt

# 配置环境变量
cp ../.env.example .env
# 编辑 .env 文件，配置必要的API密钥和数据库连接

# 启动后端服务
python -m uvicorn openfinance.api.main:app --reload --port 19100
```

#### 3. 前端设置

```bash
# 打开新终端
cd frontend

# 安装依赖
npm install

# 启动开发服务器
npm run dev
```

#### 4. 访问应用

- 前端: http://localhost:3000
- API文档: http://localhost:19100/docs
- ReDoc: http://localhost:19100/redoc

### Docker部署

```bash
# 启动所有服务
docker-compose up -d

# 查看日志
docker-compose logs -f

# 停止服务
docker-compose down
```

---

## 📁 项目结构

```
openfinance/
├── backend/                    # 后端服务
│   ├── openfinance/
│   │   ├── api/               # API路由
│   │   │   ├── routes/        # 路由模块
│   │   │   │   ├── chat.py    # 聊天API
│   │   │   │   ├── health.py  # 健康检查
│   │   │   │   ├── intent.py  # 意图识别
│   │   │   │   ├── skills.py  # 技能管理
│   │   │   │   └── tools.py   # 工具管理
│   │   │   ├── main.py        # FastAPI应用
│   │   │   └── websocket.py   # WebSocket处理
│   │   ├── agent/             # Agent模块
│   │   │   ├── core/          # 核心引擎
│   │   │   │   ├── base_agent.py
│   │   │   │   ├── graph.py   # LangGraph编排
│   │   │   │   └── state.py   # 状态管理
│   │   │   └── role/          # 角色扮演
│   │   │       └── roles.py   # 投资大师角色
│   │   ├── datacenter/        # 数据中心
│   │   │   ├── collector/     # 数据采集
│   │   │   ├── processor/     # 数据处理
│   │   │   └── provider/      # 数据服务
│   │   ├── models/            # 数据模型
│   │   │   ├── base.py        # 基础模型
│   │   │   ├── intent.py      # 意图模型
│   │   │   ├── skill.py       # 技能模型
│   │   │   └── tool.py        # 工具模型
│   │   ├── skills/            # 技能系统
│   │   │   ├── lifecycle/     # 生命周期管理
│   │   │   ├── priority/      # 优先级排序
│   │   │   ├── registry/      # 技能注册
│   │   │   └── error/         # 错误处理
│   │   └── tools/             # 工具系统
│   │       ├── registry.py    # 工具注册
│   │       ├── stock/         # 股票工具
│   │       └── market/        # 市场工具
│   ├── tests/                 # 测试文件
│   │   └── test_api_integration.py
│   ├── Dockerfile
│   └── pyproject.toml
│
├── frontend/                   # 前端应用
│   ├── app/                    # Next.js App Router
│   │   ├── finchat/           # 金融聊天页面
│   │   │   └── page.tsx
│   │   ├── layout.tsx         # 根布局
│   │   ├── page.tsx           # 首页
│   │   └── globals.css        # 全局样式
│   ├── components/            # 组件
│   │   └── ui/                # UI组件
│   │       ├── button.tsx
│   │       └── card.tsx
│   ├── lib/                   # 工具库
│   │   └── utils.ts
│   ├── services/              # 服务层
│   │   ├── apiConfig.ts       # API配置
│   │   └── FinchatServices/   # 聊天服务
│   ├── tests/                 # E2E测试
│   │   └── e2e/
│   │       ├── login.spec.ts
│   │       ├── home.spec.ts
│   │       ├── finchat.spec.ts
│   │       ├── skills.spec.ts
│   │       └── agents.spec.ts
│   ├── Dockerfile
│   ├── package.json
│   ├── playwright.config.ts
│   ├── tailwind.config.ts
│   └── tsconfig.json
│
├── .env.example               # 环境变量示例
├── docker-compose.yml         # Docker编排
├── ARCHITECTURE.md            # 架构文档
└── README.md                  # 项目说明
```

---

## 🔌 API文档

### 核心API端点

#### 健康检查

```http
GET /api/health
GET /api/health/ready
GET /api/health/live
```

#### 智能问答

```http
POST /api/chat
Content-Type: application/json

{
  "meta": {
    "trace_id": "unique-trace-id",
    "source": "web",
    "platform": "web"
  },
  "user": {
    "ldap_id": "user123",
    "user_name": "张三",
    "role": "user"
  },
  "query": "浦发银行的市盈率是多少",
  "stream": false
}
```

#### 角色对话

```http
POST /api/chat/role/{role_id}
Content-Type: application/json

{
  "query": "你怎么看比亚迪",
  "role": "warren_buffett"
}
```

#### 意图识别

```http
POST /api/intent?query=浦发银行的市盈率是多少
```

#### 工具管理

```http
GET /api/tools                    # 列出所有工具
GET /api/tools/{tool_name}        # 获取工具详情
POST /api/tools/{tool_name}/invoke # 调用工具
GET /api/tools/openai/format      # OpenAI格式工具列表
```

#### 技能管理

```http
GET /api/skills                   # 列出所有技能
GET /api/skills/{skill_id}        # 获取技能详情
GET /api/skills/stats/summary     # 技能统计
```

完整API文档请访问: http://localhost:19100/docs

---

## 🧪 测试

### 后端测试

```bash
cd backend

# 运行所有测试
python -m pytest

# 运行特定测试
python -m pytest tests/test_api_integration.py -v

# 生成覆盖率报告
python -m pytest --cov=openfinance --cov-report=html
```

### 前端测试

```bash
cd frontend

# 运行E2E测试
npm run test:e2e

# UI模式运行测试
npm run test:e2e:ui

# 运行类型检查
npm run type-check

# 运行代码检查
npm run lint
```

---

## 📦 部署指南

### 生产环境配置

1. **环境变量配置**

```bash
# .env.production
NODE_ENV=production
API_URL=https://api.openfinance.com
DATABASE_URL=postgresql://user:pass@host:5432/db
REDIS_URL=redis://host:6379
OPENAI_API_KEY=sk-xxx
```

2. **构建前端**

```bash
cd frontend
npm run build
npm run start
```

3. **构建后端**

```bash
cd backend
docker build -t openfinance-backend .
docker run -p 19100:19100 openfinance-backend
```

### Kubernetes部署

```yaml
# k8s/deployment.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: openfinance
spec:
  replicas: 3
  selector:
    matchLabels:
      app: openfinance
  template:
    spec:
      containers:
      - name: backend
        image: openfinance-backend:latest
        ports:
        - containerPort: 19100
      - name: frontend
        image: openfinance-frontend:latest
        ports:
        - containerPort: 3000
```

---

## 🔒 安全

### 认证授权

- JWT Token认证
- OAuth2.0集成
- RBAC权限控制

### 数据安全

- 敏感数据加密存储
- API密钥安全管理
- SQL注入防护
- XSS/CSRF防护

### 安全最佳实践

```python
# 环境变量管理敏感信息
from pydantic_settings import BaseSettings

class Settings(BaseSettings):
    openai_api_key: str
    database_url: str
    redis_url: str
    
    class Config:
        env_file = ".env"

settings = Settings()
```

---

## 📈 监控与日志

### 健康检查

```bash
# 检查服务状态
curl http://localhost:19100/api/health

# 就绪检查
curl http://localhost:19100/api/health/ready

# 存活检查
curl http://localhost:19100/api/health/live
```

### 日志配置

```python
import logging

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
)
```

---

## 🤝 贡献指南

我们欢迎所有形式的贡献！

### 开发流程

1. Fork 项目
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 提交 Pull Request

### 代码规范

- Python: 遵循 PEP 8 规范
- TypeScript: 使用 ESLint + Prettier
- 提交信息: 遵循 Conventional Commits

---

## 📄 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

---

## 📞 联系方式

- 项目主页: https://github.com/your-org/openfinance
- 问题反馈: https://github.com/your-org/openfinance/issues
- 邮箱: openfinance@example.com

---

## 🙏 致谢

感谢以下开源项目：

- [FastAPI](https://fastapi.tiangolo.com/)
- [Next.js](https://nextjs.org/)
- [LangChain](https://langchain.com/)
- [LangGraph](https://langchain-ai.github.io/langgraph/)
- [AKShare](https://akshare.akfamily.xyz/)
- [Tushare](https://tushare.pro/)

---

<div align="center">

**⭐ 如果这个项目对你有帮助，请给一个 Star ⭐**

Made with ❤️ by OpenFinance Team

</div>
